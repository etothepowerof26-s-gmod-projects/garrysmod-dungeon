...
function ENT:ChaseEnemy(options)
    ...
    while (path:IsValid()) do
        ...
        --	lastSoundDur = lastSoundDur / 3
        --end
        -- and SysTime() > self.LastSound + lastSoundDur then
        if en:GetPos():DistToSqr(self:GetPos()) < d and not self.attacking then
            self:DoAttack()
            break
        end
...
function ENT:DoAttack()
    local do_03 = math.random(1, 2) == 2
    local lookup = self:LookupSequence(do_03 and "morbus_pestis_attack_punch_02_combo" or "morbus_pestis_attack_punch_01") --:IsPlayingGesture(
    ...
    local function check()
        ...
        if en:GetPos():DistToSqr(self:GetPos()) < d * 2 then
            ...
            d:SetInflictor(self)
            en:TakeDamageInfo(d)
            en:SetVelocity(en:GetVelocity() + self:GetForward() * 450 + Vector(0, 0, 400))
