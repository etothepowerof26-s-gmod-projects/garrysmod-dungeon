...
local ply_w_presses = setmetatable({}, {
    __mode = "k"
})
...
local function reinit()
    GAMEMODE.WarriorDashCache = setmetatable({}, {
        __mode = "k"
    })
end
...
hook.Add("KeyPress", "keypress_use_hi", function(ply, key)
    ...
    if (class and class.Name == "Warrior" and key == IN_FORWARD and ply:GetPLevel() > 4 and not ply:IsBot()) then
        local TIME = 0.35
